...
public class SubroutineHierarchy extends JPanel {
	...
	private void drawWorld(Graphics2D g2) {
		...
		rotatingPoly(g2,0.015,0.015,-0.7,0.04);//koła na środku (obracające się)
		rotatingPoly(g2,0.015,0.015,1.5,-0.8);

		rotatingPoly(g2,0.013,0.013,-3.3,1.65); //koła na lewo (obracające się)
		rotatingPoly(g2,0.013,0.013,-1.6,1);

		rotatingPoly(g2,0.01,0.01,1.25,2.1);//koła na prawo (obracające się)
		rotatingPoly(g2,0.01,0.01,2.6,1.65);

		filledRect(g2,0.95,0.08,-0.25,-0.15); //tworzenie prostokątów
		filledRect(g2,0.75,0.07,-3,1.5);
		filledRect(g2,0.55,0.06,1.5,2);

		filledTriangle(g2,Color.blue,0.5,1.5,0.4,-1.8); //tworzenie podstaw figur - trójkąty
		filledTriangle(g2,new Color(0x800080),0.4,1.3,-2.5,0.1);
		filledTriangle(g2,new Color(0x228B22),0.3,1.2,1.85,0.7);
	...
	private void rotatingPoly(Graphics2D g2, double skalaX, double skalaY, double translateX, double translateY) {
		AffineTransform saveTransform = g2.getTransform(); //zmiana punktu 0,0
		Color saveColor = g2.getColor();
		g2.translate(translateX, translateY); //ustawienie osi X i Y

		var N = 12; //zmienna N (dwunastokąt)
		int[] x = new int[N];
		int[] y = new int[N];
		...
		} //wzór do rysowania wielokąta

		Polygon polygon = new Polygon(x, y, N); //zmienna do rysowania wielokąta
		g2.setColor(new Color(0xFFA500)); //ustawienie koloru pomarańczowego
		g2.rotate(Math.toRadians(frameNumber * 0.75)); //aktualizacja przed narysowaniem kolejnej klatki
		g2.scale(skalaX,skalaY); //zmiana wielkości
		g2.fill(polygon); //wypełnienie wielokąta
		g2.setColor(saveColor);
		g2.setTransform(saveTransform); //miejsce figury
	}
	...
	private static void filledRect(Graphics2D g2, double skalaX, double skalaY, double translateX, double translateY) { // Fills a square, size = 1, center = (0,0)
		AffineTransform saveTransform = g2.getTransform();  //filledRect - funkcja do tworzenia prostokątów
		Color saveColor = g2.getColor();
		g2.setColor(Color.red); //ustawienie koloru na czerwony

		g2.translate(translateX, translateY); //przesunięcie
		g2.rotate(Math.toRadians(-20)); //obrót ze stopni na Radiany
		g2.scale(skalaX,skalaY); //zmiana wielkości

		g2.fill(new Rectangle2D.Double(-0.5,-0.5,2.5,2.24)); //wypełnienie prostokąta

		g2.setColor(saveColor); //ustawienie koloru
	...
	private static void filledTriangle(Graphics2D g2,Color color, double skalaX, double skalaY, double translateX, double translateY) { // width = 1, height = 1, center of base is at (0,0);
		Path2D path = new Path2D.Double(); //Nowa ścieżka
		...
		path.moveTo(-0.5, 0); //przesunięcie punktu
		path.lineTo(0.5, 0);
		path.lineTo(0, 1);
		g2.translate(translateX, translateY);
		g2.scale(skalaX,skalaY);
		path.closePath(); //zakończenie ścieżki
		g2.fill(path); //wypełnienie stworzonego przez ściężkę elementu
